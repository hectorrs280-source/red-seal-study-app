...
(function () {
  ...
  function inc(bucket, id, correct) { bucket[id] ||= { answered: 0, correct: 0 }; bucket[id].answered += 1; if (correct) bucket[id].correct += 1; }
  function recordQuizAnswer(question, correct, detail = {}) { const p = load(); p.quiz.answered += 1; if (correct) p.quiz.correct += 1; inc(p.quiz.byMwa, question.mwa, correct); inc(p.quiz.byTask, question.task, correct); p.quiz.history.push({ questionId: question.id, mwa: question.mwa, task: question.task, subtask: question.subtask, questionType: question.question_type, correct, selectedIndex: detail.selectedIndex, correctIndex: question.correct, mode: detail.mode || 'practice', at: new Date().toISOString() }); p.quiz.history = p.quiz.history.slice(-1500); touch(p); save(p); return p; }
  ...
  function markDay(dayIndex, done = true) { const p = load(); const id = Number(dayIndex); const set = new Set(p.calendar.completedDays); done ? set.add(id) : set.delete(id); p.calendar.completedDays = [...set].sort((a,b)=>a-b); touch(p); save(p); }
  ...
  function streak(dates) { const s = new Set(dates); let d = new Date(); let n = 0; if (!s.has(dateKey(d))) d = new Date(d.getTime() - DAY); while (s.has(dateKey(d))) { n += 1; d = new Date(d.getTime() - DAY); } return n; }
  ...
  function stats() { const p = load(); const mastered = Object.values(p.vocabulary).filter(v => v.repetitions >= 3 && v.easeFactor >= 2.3).length; return { ...p, mastered, streak: streak(p.activityDates), weakTasks: weakTasks(6) }; }
